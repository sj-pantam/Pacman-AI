...
class CornersProblem(SearchProblem):
    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if (not hitsWall):
                nextP = (nextx, nexty)
                if nextP in visited_L:
                    visited_L.remove(nextP)
                successors.append(((nextP, tuple(visited_L)), action, 1))
        return successors
...
def cornersHeuristic(state, problem):
    ...
    if len(corners) == 0:
        ...
        problem.goal = corners[len(corners) - 1]
...
def foodHeuristic(state, problem):
    ...
    if len(food_coord) == 0:
        ...
        problem.goal = food_coord[len(food_coord) - 1]
    return heuristic.manhattan(state[0], problem)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # *** Your Code Here ***
        problem = AnyFoodSearchProblem(gameState, start = gameState.getPacmanPosition())
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoal(self, state):
        return bool(state in self.food.asList())
